...
Define `@prefix us_wbd: <https://wbd.spatialai.org/v1/wbd#>`

Define `@prefix us_wbd_data: <https://wbd.spatialai.org/v1/wbd-data#>`

| Instance Class | IRI Format |
| --- | --- |
| `us_wbd:HU02` <br/> `us_wbd:HU04` <br/> `us_wbd:HU06` <br/> `us_wbd:HU08` <br/> `us_wbd:HU10` <br/> `us_wbd:HU12` | `wbd_gcx:hu02/<2-digit id>` <br/> `wbd_gcx:hu04/<4-digit id>` <br/> `wbd_gcx:hu06/<6-digit id>` <br/> `wbd_gcx:hu08/<8-digit id>` <br/> `wbd_gcx:hu10/<10-digit id>` <br/> `wbd_gcx:hu12/<12-digit id>` |
| `geo:Geometry` | `us_wbd_data:d.HU02.<2-digit id>.geometry` <br/> `us_wbd_data:d.HU04.<4-digit id>.geometry` <br/> `us_wbd_data:d.HU06.<6-digit id>.geometry` <br/> `us_wbd_data:d.HU08.<8-digit id>.geometry` <br/> `us_wbd_data:d.HU10.<10-digit id>.geometry` <br/> `us_wbd_data:d.HU12.<12-digit id>.geometry` |
...
| states | The States or outlying area attribute identifies the State(s) or outlying areas that the hydrologic unit falls within or touches. | Yes | `us_wbd:hucState` | Object property connecting to Administrative Region 1 instances |
| huc<#> | The HUC# field is a unique #-digit hydrologic unit code <br> # is 2 (region), 4 (subregion), 6 (basin), 8 (subbasin), 10 (watershed), or 12 (subwatershed) | Yes | `us_wbd:hucCode` | Use as unique identifier |
...
| hutype | The 10-digit hydrologic unit type attribute is the single-letter abbreviation for Watershed type from the list of official names provided in the WBD Standards. | Yes | `us_wbd:hucType` | HUC10+ only <br/> See controlled vocabulary below |
...
| hutype_description |  | Yes | `us_wbd:hucTypeDescription` | HUC10+ only <br/> See controlled vocabulary below |
| tohuc | The 12-digit hydrologic unit ToHUC code attribute is the code for the 12-digit hydrologic unit that is downstream from and naturally receives the majority of the flow from another 12-digit hydrologic unit. | Yes | `us_wbd:toHUC` | HUC12+ only |
