...
ttl_file = 'il_saw_aquifers_aqsystems.ttl'
...
logname = 'log_IL_ISGS_Aquifers-2-ttl_aqsystems.txt'
...
def build_aqsys_iris(aqsysid, _PREFIX):
    ...
    :return: a tuple with the two IRIs
    """
    return (_PREFIX["il_isgs_data"]['d.ISGS-Aquifer-System.CM' + str(aqsysid).zfill(max_id_length)],
            _PREFIX["il_isgs_data"]['d.ISGS-Aquifer-System.Geometry.CM' + str(aqsysid).zfill(max_id_length)])


def process_aquifers_shp2ttl(br_infile, sg_infile, cg_infile, cgsys_infile, outfile, ids_dict):
    ...
    for gdf in list_aqs:
        ...
        for row in gdf.itertuples():
            ...
            kg.add((aqiri, _PREFIX['il_isgs']['ilSawAqId'], Literal(aqid, datatype=XSD.string)))
            kg.add((aqiri, _PREFIX['saw_water']['aquiferType'], Literal(row.aqtype, datatype=XSD.string)))
            kg.add((aqiri, RDFS.label, Literal(f'A {row.aqtype} aquifer in Illinois', lang='en')))
            if row.aqtype.lower() == 'coarse-grain_materials':
                sysid = str(ids_dict[fid])
                aqsysiri = build_aqsys_iris(sysid, _PREFIX)[0]
                kg.add((aqiri, _PREFIX['gwml2']['gwAquiferSystem'], aqsysiri))
                kg.add((aqsysiri, _PREFIX['gwml2']['gwAquiferSystemPart'], aqiri))
            kg.add((aqiri, GEO.hasGeometry, geoiri))
            kg.add((aqiri, GEO.defaultGeometry, geoiri))
            kg.add((geoiri, GEO.asWKT, Literal(row.geometry, datatype=GEO.wktLiteral)))
            kg.add((geoiri, RDF.type, GEO.Geometry))
            if 'multipolygon' in str(row.geometry).lower():
                kg.add((geoiri, RDF.type, _PREFIX['sf']['MultiPolygon']))
            else:
                kg.add((geoiri, RDF.type, _PREFIX['sf']['Polygon']))
    logger.info(f'   Processing {ordinals[count]} set of aquifers (of four)')
    for row in gdf_coarsemtls_aqsys.itertuples():
        aqsysiri, geoiri = build_aqsys_iris(row.saw_id, _PREFIX)
        kg.add((aqsysiri, RDF.type, _PREFIX['gwml2']['GW_AquiferSystem']))
        kg.add((aqsysiri, _PREFIX['il_isgs']['ilSawAqSysId'],
                Literal('CM' + str(row.saw_id).zfill(max_id_length), datatype=XSD.string)))
        kg.add((aqsysiri, RDFS.label, Literal('A system of aquifers in Illinois', lang='en')))
        kg.add((aqsysiri, RDFS.comment, Literal(
            ...
            lang='en')))
        kg.add((aqsysiri, GEO.hasGeometry, geoiri))
        kg.add((aqsysiri, GEO.defaultGeometry, geoiri))
        kg.add((geoiri, GEO.asWKT, Literal(row.geometry, datatype=GEO.wktLiteral)))
        kg.add((geoiri, RDF.type, GEO.Geometry))
        if 'multipolygon' in str(row.geometry).lower():
            kg.add((geoiri, RDF.type, _PREFIX['sf']['MultiPolygon']))
...
if __name__ == '__main__':
    ...
    process_aquifers_shp2ttl(isgs_bedrocklt500_aq_shp_path, isgs_sandgravel_aq_shp_path, isgs_coarsemtls_aq_shp_path,
                             isgs_coarsemtls_aqsys_shp_path, ttl_file, ids)
